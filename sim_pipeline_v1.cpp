...
struct Args {
    ...
    std::string log_jsonl{"output/sim_v1_log.jsonl"};
    int device{0};
};
...
static Args parseConfig(int argc, char** argv) {
    ...
    a.log_jsonl = getArg(kv, "--log-jsonl", "output/sim_v1_log.jsonl");
    a.device = std::max(0, getArgInt(kv, "--device", 0));
...
int main(int argc, char** argv) {
    try {
        ...
        auto worker_fn = [&](const std::string& engine_path, int requested_bs, BoundedQueue<cv::Mat>& q) {
            TrtRunner runner(engine_path, args.device);
        ...
        threads.emplace_back(worker_fn, args.det_engine, args.det_batch_size, std::ref(det_queue));
        threads.emplace_back(worker_fn, args.seg_engine, args.seg_batch_size, std::ref(seg_queue));
