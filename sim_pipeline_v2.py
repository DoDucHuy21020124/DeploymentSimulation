...
def source_loop(name: str, seed_paths: List[Path], out_q: queue.Queue, fps: float, stop_event: threading.Event) -> None:
    idx = 0
    ...
    while not stop_event.is_set():
        ...
        if interval > 0:
            ...
            # print(f'Time difference: {next_deadline - now}')
...
def worker_loop(
    ...
    while not stop_event.is_set():
        ...
        # Pop-only lock: dequeue serialized, inference concurrent.
        with pop_lock:
...
def logger_loop(
    ...
    with log_jsonl.open("a", encoding="utf-8") as f:
        while not stop_event.is_set():
            ...
            # Logger must contain only these two values.
            line = json.dumps(rec)
...
def main() -> None:
    ap = argparse.ArgumentParser("TensorRT simulation pipeline v2 (multi-worker)")
    ...
    ap.add_argument("--log-jsonl", type=str, default="output/sim_v2_log.jsonl")
    ap.add_argument("--num-det-workers", type=int, default=6)
    ap.add_argument("--num-seg-workers", type=int, default=6)
    ap.add_argument("--gpu-ids", type=str, default="0,1,2,3,4,5")
    args = ap.parse_args()
    ...
    stop_event = threading.Event()
    pop_lock_det = threading.Lock()
    pop_lock_seg = threading.Lock()
    ...
    det_workers = max(1, args.num_det_workers)
    seg_workers = max(1, args.num_seg_workers)

    assert det_workers == seg_workers == len(gpu_ids), "Number of workers must match number of GPU IDs"
    ...
    threads.append(
        threading.Thread(
            target=source_loop,
            args=("det_source", det_seed, det_q, args.det_source_fps, stop_event),
            daemon=True,
        )
    )
    threads.append(
        threading.Thread(
            target=source_loop,
            args=("seg_source", seg_seed, seg_q, args.seg_source_fps, stop_event),
            daemon=True,
        )
    )

    for i in range(det_workers):
        gpu = gpu_ids[i % len(gpu_ids)]
        gpu = f"cuda:{gpu}"
        x = torch.rand((1, 3, 480, 640)).to(gpu)
        del x  # warm up CUDA context
        model = TrtModelWorker(args.det_engine, gpu, args.det_batch_size)
        threads.append(
            threading.Thread(
                target=worker_loop,
                args=(model, det_q, pop_lock_det, stop_event),
                daemon=True,
                name=f"det_worker_{i}",
            )
        )

    for i in range(seg_workers):
        gpu = gpu_ids[i % len(gpu_ids)]
        x = torch.rand((1, 3, 480, 640)).to(gpu)
        del x  # warm up CUDA context
        model = TrtModelWorker(args.seg_engine, gpu, args.seg_batch_size)
        threads.append(
            threading.Thread(
                target=worker_loop,
                args=(model, seg_q, pop_lock_seg, stop_event),
                daemon=True,
                name=f"seg_worker_{i}",
            )
        )
    ...
    threads.append(
        threading.Thread(
            target=logger_loop,
            args=(det_q, seg_q, log_q, Path(args.log_jsonl), stop_event),
            daemon=True,
            name="logger",
        )
    )
